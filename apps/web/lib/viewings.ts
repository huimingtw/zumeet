...
export function formatSlot(startISO: string, endISO: string): string {
  ...
  const day = s.toLocaleDateString("zh-TW", { ...opts, month: "numeric", day: "numeric", weekday: "short" });
  const t = (d: Date) => d.toLocaleTimeString("zh-TW", { ...opts, hour: "2-digit", minute: "2-digit", hour12: false });
  return `${day} ${t(s)}–${t(e)}`;
}
